  ...
  const scheduleResult = useMemo(() => {
    if (selectedDay && selectedTime) {
      return {
        day: selectedDay,
        start: selectedTime,
        end: moment(selectedTime).add(selectedServices.reduce((total, { duration }) => total + duration, 0), 'minutes'),
      }
    }
        ...
        <S.Column gap='4px'>
          {selectedDay && (
            <>
              ...
              <S.Row gap='4px'>
                {[...availableTimes, moment().add(35, 'minutes')].map((time) => (
                  <S.TimeView key={time} onPress={() => handleTimePress(time)}>
        ...
        {scheduleResult && (
          <S.Column gap='12px'>
            <S.Column gap='8px'>
              ...
              <S.HeaderSubTitle>{selectedServices.map(({ name }) => name).join(', ')}</S.HeaderSubTitle>
              <S.HeaderSubTitle>{scheduleResult.day.format('DD/MM/YYYY')}: {scheduleResult.start.format('HH:mm')} - {scheduleResult.end.format('HH:mm')}</S.HeaderSubTitle>
            </S.Column>
